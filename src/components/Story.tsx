...
import { STORY_IMAGE } from '../utils/imageAssets';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          <div className={`lg:col-span-7 space-y-6 ${language === 'ar' ? 'text-right' : 'text-left'}`}>
            ...
            <p className="text-gray-600 text-sm sm:text-base leading-relaxed">
              {language === 'ar' ? (
                <>
                  تعاونية <strong>تعاونية تدمامت (Coopérative Tadmamte)</strong> ليست مجرد مشروع تجاري، بل هي حلم جماعي للأسر والنساء في قرى سهول وواحات سوس المعطاء. هنا، حيث تلتقي أصالة الأرض بجمالها، تجتمع نساؤنا وعضواتنا لنقل المعرفة المتوارثة جيلاً بعد جيل في إنتاج أخلص هدايا الأرض الطبيعية.
                </>
              ) : language === 'fr' ? (
                <>
            ...
            <p className="text-gray-600 text-sm sm:text-base leading-relaxed">
              {language === 'ar' ? (
                <>
                  كل قارورة عسل تقتنيها وكل علبة أملو توضع على مائدتك هي نتاج أيام طويلة من العمل الدؤوب والمحبة الفائقة. نحن لا نستخدم الآلات الصناعية لتسريع الإنتاج على حساب القيمة، بل نحافظ بكل فخر على طحن اللوز برحى الحجر اليدوية التقليدية وعصر الأركان على البارد واستخلاص العسل بطرق صديقة للبيئة تحترم النحل والطبيعة لتقديم منتجات طبيعية خالصة.
                </>
              ) : language === 'fr' ? (
                <>
                  Chaque pot de miel que vous achetez et chaque boîte d'amlou sur votre table est le fruit de longues journées de travail dévoué. Nous n'utilisons pas de machines industrielles au détriment de la qualité, mais nous perpétuons fièrement le moulin de pierre traditionnel, le pressage à froid de l'argan et l'extraction écologique du miel respectueuse de la nature.
                </>
              ) : (
                <>
                  Every jar of honey you purchase and every box of amlou on your table is the result of long days of dedicated labor and love. We do not use industrial machinery at the expense of value; we proudly preserve traditional stone-milling, cold-pressed argan extraction, and eco-friendly honey harvesting that respects bees and nature.
                </>
              )}
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-6 border-t border-brand-sand">
              ...
              <div className="space-y-2 bg-brand-sand/30 p-4 rounded-2xl border border-brand-brown/5">
                ...
                <h4 className="font-reem font-bold text-brand-brown text-sm">
                  {language === 'ar' ? 'أرض معقمة وخالصة' : language === 'fr' ? 'Terre Pure & Préservée' : 'Pure & Preserved Land'}
                </h4>
                <p className="text-xs text-gray-500 leading-normal">
                  {language === 'ar' 
                    ? 'مناحلنا تقع على ارتفاعات شاهقة بعيدة عن المدن والملوثات، مع استخلاص العسل بطرق صديقة للبيئة.' 
                    : language === 'fr' 
                    ? 'Nos ruchers se trouvent en haute altitude, loin de toute pollution avec une extraction écologique.' 
                    : 'Our apiaries are in high altitudes, far from pollution with eco-friendly extraction.'}
                </p>
              </div>
          ...
          <div className="lg:col-span-5 relative">
            <div className="relative z-10 rounded-3xl overflow-hidden w-full aspect-[4/3] min-h-[260px] shadow-2xl border-4 border-brand-sand bg-brand-sand/50 flex items-center justify-center">
              <img
                src={STORY_IMAGE.webp || STORY_IMAGE.jpg}
                ...
                onError={(e) => {
                  ...
                  e.currentTarget.src = STORY_IMAGE.jpg || '/images/cooperative_hands_craft.jpg';
                }}
              />
              <div className="absolute inset-0 bg-linear-to-t from-brand-dark/25 to-transparent" />
            </div>
