...
import { resolveAvatarImage } from '../utils/imageAssets';
  ...
  const data: Record<string, Record<'ar' | 'fr' | 'en', { name: string; role: string; location: string; content: string }>> = {
    ...
    't-2': {
      ar: {
        ...
        location: 'أكادير',
        content: 'أشتري عسل السدر الجبلي والزعتر من تعاونية تدمامت (Coopérative Tadmamte) بانتظام. منتجات طبيعية خالصة والذوق لا تشوبه شائبة، وتغليف المنتجات في قوارير زجاجية أنيقة يشعرك بالفخامة قبل تذوقه. الشحن سريع ومهذب جداً.'
      },
      fr: {
              ...
              >
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-brand-sand mt-auto">
                  <div className="w-10 h-10 shrink-0">
                    {(() => {
                      const avatarAssets = resolveAvatarImage(test.id, test.avatar);
                      return (
                        <img
                          src={avatarAssets.webp || avatarAssets.jpg}
                          ...
                          onError={(e) => {
                            ...
                            e.currentTarget.src = avatarAssets.jpg || avatarAssets.webp || '/images/avatar_amina.jpg';
                          }}
                        />
                      );
                    })()}
