...
export function AdminPanel({
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (file.size > 15 * 1024 * 1024) {
        setFormError(isAr ? 'حجم الصورة يتجاوز الحد الأقصى المسموح به (15 ميغابايت)' : 'Image size exceeds maximum limit of 15MB');
        if (fileInputRef.current) fileInputRef.current.value = '';
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Upload image to Firebase Storage if a new file is uploaded
      ...
      if (imageFile) {
        ...
        // Cleanup old image if replacing image during edit
        if (existingProduct && existingProduct.image && existingProduct.image !== finalImageUrl && existingProduct.image.includes('firebasestorage.googleapis.com')) {
          deleteProductImage(existingProduct.image).catch(err => console.warn('Old image deletion warning:', err));
        }
      }
      ...
      if (editingId) {
        ...
        showSuccess(isAr ? 'تم تعديل المنتج وحفظ الصورة بنجاح' : language === 'fr' ? 'Produit modifié avec succès' : 'Product updated successfully');
      } else {
        onAddProduct(productData);
        showSuccess(isAr ? 'تمت إضافة المنتج الجديد وحفظ الصورة بنجاح' : language === 'fr' ? 'Nouveau produit ajouté avec succès' : 'New product added successfully');
      }
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-brand-sand flex flex-col min-h-screen" dir={isAr ? 'rtl' : 'ltr'}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className={isAr ? 'text-right' : 'text-left'}>
            ...
            <p className="text-[10px] text-brand-gold font-medium">
              {isAr ? 'إدارة مخزون Coopérative Tadmamte' : 'Inventory Management - Coopérative Tadmamte'}
            </p>
          </div>
        </div>
                ...
                <div className={`grid grid-cols-1 md:grid-cols-12 gap-6 ${isAr ? '' : 'direction-ltr'}`}>
                  ...
                  <div className="md:col-span-4 flex flex-col gap-3">
                    ...
                    <div 
                      ...
                      className="border-2 border-dashed border-brand-brown/20 hover:border-brand-gold rounded-2xl aspect-4/3 flex flex-col items-center justify-center p-4 bg-brand-sand/10 hover:bg-brand-sand/30 cursor-pointer transition-all overflow-hidden relative"
                    >
                      {imagePreview ? (
                        <>
                          <img 
                          ...
                          <td className="py-3">
                            <div className="relative group/img w-12 h-12">
                              <img 
                                src={prod.image} 
                                alt={displayName} 
                                className="w-12 h-12 object-cover rounded-lg border border-brand-brown/10 bg-brand-sand/50"
      ...
      <footer className="bg-brand-brown text-gray-400 text-center py-4 text-xs border-t border-brand-gold/5 mt-auto">
        {isAr 
          ? 'جميع الصلاحيات محفوظة لمشرفي Coopérative Tadmamte ⵣ.' 
