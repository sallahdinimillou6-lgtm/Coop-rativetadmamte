...
export function CartDrawer({
  ...
  const handleWhatsAppCheckout = () => {
    ...
    if (language === 'ar') {
      ...
      const shippingText = shippingCost === 0 ? 'شحن مجاني' : `${shippingCost} ${unit}`;

      text = `السلام عليكم ورحمة الله،\nأود تأكيد طلبي من موقع *Coopérative Tadmamte ⵣ*:\n\n*📋 تفاصيل المنتجات:*\n${itemsText}\n\n*🚚 التوصيل:* ${shippingText}\n*💵 المبلغ الإجمالي الدفع عند الاستلام:* ${finalTotal} ${unit}\n\n*📍 معلومات التوصيل:*\n• الاسم الكامل: \n• المدينة: \n• العنوان: \n\nشكراً لكم وفي انتظار تأكيد الطلب 🙏`;
    } else if (language === 'fr') {
      const itemsText = cartItems.map(item => {
      ...
      const shippingText = shippingCost === 0 ? 'Livraison Gratuite' : `${shippingCost} ${unit}`;

      text = `Bonjour,\nJe souhaite confirmer ma commande sur le site *Coopérative Tadmamte ⵣ* :\n\n*📋 Détails des articles :*\n${itemsText}\n\n*🚚 Livraison :* ${shippingText}\n*💵 Total à payer à la livraison :* ${finalTotal} ${unit}\n\n*📍 Informations de livraison :*\n• Nom complet : \n• Ville : \n• Adresse : \n\nMerci et en attente de votre confirmation 🙏`;
    } else {
      const itemsText = cartItems.map(item => {
      ...
      const shippingText = shippingCost === 0 ? 'Free Shipping' : `${shippingCost} ${unit}`;

      text = `Hello,\nI would like to confirm my order from *Coopérative Tadmamte ⵣ*:\n\n*📋 Order details:*\n${itemsText}\n\n*🚚 Shipping:* ${shippingText}\n*💵 Total (Cash on Delivery):* ${finalTotal} ${unit}\n\n*📍 Delivery Information:*\n• Full Name: \n• City: \n• Address: \n\nThank you, awaiting your confirmation 🙏`;
    }
                  ...
                  <div className="flex flex-col gap-4">
                    {cartItems.map(item => {
                      ...
                      const imageAssets = resolveProductImage(
                        ...
                        item.product.description || item.product.descriptionAr
                      );
                      ...
                      return (
                        ...
                        >
                          <div className="w-16 h-16 shrink-0 flex items-center justify-center bg-white rounded-xl border border-brand-brown/5 p-1 shadow-2xs overflow-hidden">
                            <img
